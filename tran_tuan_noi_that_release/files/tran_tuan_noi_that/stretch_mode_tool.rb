  ...
  module StretchMode
    ...
    class Tool
      ...
      def commit_resize
        ...
        begin
          # Component gốc có nhiều instance: tách riêng đúng module đang chọn trước khi sửa.
      ...
      def collect_actions(entities, parent_to_root, actions, axis, sign, delta,
        ...
        # Raw geometry trong scope hiện tại: co theo tỷ lệ quanh mép cố định.
        ...
        children.each do |child|
          ...
          root_transform = if near_moving && !near_fixed && span_ratio < FULL_SPAN_RATIO
                             # Tấm mỏng ở mép đang kéo: chỉ di chuyển, giữ nguyên độ dày.
                             translation_transform(axis, delta)
                           elsif near_fixed && !near_moving && span_ratio < FULL_SPAN_RATIO
                             # Tấm mỏng ở mép neo: giữ nguyên vị trí.
                             nil
                           elsif (near_fixed && near_moving) || span_ratio >= FULL_SPAN_RATIO
                             # Tấm chạy dài theo trục: kéo dài quanh mép cố định.
                             scale_transform(axis, fixed_coord, ratio)
                           else
                             # Tấm/ngăn bên trong: giữ kích thước, dịch theo tỷ lệ khoảng trống.
