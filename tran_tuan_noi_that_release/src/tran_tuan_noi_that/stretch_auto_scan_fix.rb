    ...
    class Tool
      ...
      def onLButtonUp(flags, x, y, view)
        if @input_mode == :auto_scan && @auto_dragging && @state == :p2
          @auto_dragging = false
          @auto_scan_current_2d = Geom::Point3d.new(x.to_f, y.to_f, 0)
          distance = @auto_scan_start_2d ? @auto_scan_start_2d.distance(@auto_scan_current_2d) : 0.0
      ...
      def draw_auto_selected_halfspace(view, region)
        ...
        if respond_to?(:box_faces, true)
          ...
          box_faces(bounds).each { |face| view.draw(GL_QUADS, face.map { |p| root_to_world(p) }) }
        end
