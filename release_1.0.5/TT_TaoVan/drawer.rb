module TranTuan
  module TaoVan
    module Drawer
      ...
      # QUY ƯỚC ĐƠN VỊ GIAO TIẾP: mm.
      # SketchUp API dùng inch nội bộ; chỉ chuyển mm -> inch tại ranh giới tạo hình học.
      def mm(v)
        v.to_f.mm
      end
      ...
      class TwoPointTool
        ...
        def onMouseMove(_flags, x, y, view)
          ...
          if @p1
            ...
            Sketchup.set_status_text(
              "TỰ ĐỘNG 2 ĐIỂM | ĐIỂM 1: MẶT TRÊN → ĐIỂM 2: ĐÁY | Cao: #{Drawer.mm_text(h_mm)} | ĐƠN VỊ: mm | TAB = THỦ CÔNG",
              SB_VCB_LABEL
            )
          end
        end
        ...
        def draw(view)
          ...
          if @manual_mode && @manual_ready && @manual_values
            ...
            oy = @manual_origin.y
            oz = @manual_origin.z
            w_mm, d_mm, h_mm = @manual_values[0, 3]
            draw_box(view, box_points(ox, oy, oz, Drawer.mm(w_mm), Drawer.mm(d_mm), Drawer.mm(h_mm)))
            return
          end
          ...
          x0 = bb ? bb.min.x : [a.x, b.x].min
          x1 = bb ? bb.max.x : [a.x, b.x].max
          y0 = bb ? bb.min.y : [a.y, b.y].min
          y1 = bb ? bb.max.y : [a.y, b.y].max
        ...
        def set_waiting_status(mode, detail)
          Sketchup.set_status_text(
            "TT NGĂN KÉO | CHẾ ĐỘ CHỜ | #{mode} | ĐƠN VỊ: mm | TAB = ĐỔI CHẾ ĐỘ",
            SB_PROMPT
          )
        ...
        def update_status
          if @manual_mode
            detail = if @manual_ready
              'ĐÃ NHẬP THÔNG SỐ (mm) → DI CHUỘT ĐỂ XEM PREVIEW → CLICK ĐỂ TẠO'
            else
        ...
        def box_points(x, y, z, w, d, h)
          [
            ...
            Geom::Point3d.new(x, y, z + h),
            Geom::Point3d.new(x + w, y, z + h),
            Geom::Point3d.new(x + w, y + d, z + h),
            Geom::Point3d.new(x, y + d, z + h)
          ]
        end
        ...
        def same_container?(a, b)
          ...
          a.equal?(b) || (
            a.respond_to?(:persistent_id) && b.respond_to?(:persistent_id) &&
        ...
        def manual_create
          ...
          unless [w, d, h, t, bt].all? { |v| v.finite? && v > 0 } &&
                 [gl, gf, bo].all? { |v| v.finite? && v >= 0 }
            UI.messagebox('Thông số không hợp lệ. Tất cả kích thước phải nhập bằng mm.')
            return
          end
        ...
        def create_manual_from_preview
          ...
          create_drawer(
            ...
            @manual_origin.x, @manual_origin.y, @manual_origin.z,
            w, d, h, t, bt, gl, gf, bo
          )
        end

        def create_drawer(model, ox, oy, oz, w_mm, d_mm, h_mm, t_mm, bt_mm, gl_mm, gf_mm, bo_mm = 0)
          # Từ đây trở xuống: mọi biến *_mm vẫn là mm cho đến ngay trước khi tạo hình học.
          w_mm  = w_mm.to_f
          d_mm  = d_mm.to_f
          h_mm  = h_mm.to_f
          t_mm  = t_mm.to_f
          ...
          if iw_mm <= 0 || id_mm <= 0 || h_mm <= t_mm
            UI.messagebox("Kích thước khoang không đủ cho ván #{t_mm.round(1)} mm và khe hở đã nhập. Đơn vị: mm")
            reset
            return
          end

          # Chuyển đúng một lần từ mm sang đơn vị nội bộ SketchUp.
          w  = Drawer.mm(w_mm)
          d  = Drawer.mm(d_mm)
          h  = Drawer.mm(h_mm)
          t  = Drawer.mm(t_mm)
          ...
          outer.set_attribute('TT_TaoVan', 'rong_mm', w_mm)
          outer.set_attribute('TT_TaoVan', 'sau_mm', d_mm)
          outer.set_attribute('TT_TaoVan', 'cao_mm', h_mm)
          outer.set_attribute('TT_TaoVan', 'day_mm', t_mm)
          outer.set_attribute('TT_TaoVan', 'day_da_mm', bt_mm)

          model.commit_operation
          model.selection.clear
          model.selection.add(outer)
          Sketchup.set_status_text(
            "Đã tạo ngăn kéo: #{mm_text(w_mm)} × #{mm_text(d_mm)} × #{mm_text(h_mm)}",
            SB_PROMPT
          )
          reset
        rescue => e
