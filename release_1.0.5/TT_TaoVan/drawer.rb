    ...
    module Drawer
      ...
      class TwoPointTool
        ...
        def draw(view)
          ...
          x0=bb ? bb.min.x : [a.x,b.x].min; x1=bb ? bb.max.x : [a.x,b.x].max
          y0=bb ? bb.min.y : [a.y,b.y].min; y1=bb ? bb.max.y : [a.y,b.y].max
        ...
        def onLButtonDown(_flags,x,y,view)
          ...
          if @manual_mode
            unless @manual_ready
              @manual_origin=p; manual_create; return
            end
          ...
          if @p1.nil?
            ...
            unless @container
              UI.messagebox('Điểm 1 phải nằm trên Face thuộc Group/Component của khoang tủ.'); reset; return
            end
            set_waiting_status('TỰ ĐỘNG 2 ĐIỂM','ĐIỂM 1 ĐÃ NHẬN → DI CHUỘT XUỐNG MẶT ĐÁY → CLICK ĐIỂM 2')
            view.invalidate
          else
            @p2=p; create_from_two_points
          end
        end

        def onKeyDown(key,_repeat,_flags,view)
          if key == 9
            @manual_mode=!@manual_mode; clear_state; update_status; view.invalidate if view
          elsif key == 27
        ...
        def set_waiting_status(mode,detail)
          Sketchup.set_status_text("TT NGĂN KÉO | CHẾ ĐỘ CHỜ | #{mode} | TAB = ĐỔI CHẾ ĐỘ",SB_PROMPT)
        ...
        def create_from_two_points
          ...
          unless same_container?(@container,c2)
            UI.messagebox('Điểm 2 phải nằm trên Face thuộc cùng Group/Component với điểm 1.'); reset; return
          end
          bb=@container.bounds; w=bb.width.to_mm; d=bb.depth.to_mm
          if w<=1 || d<=1
            UI.messagebox('Không xác định được Rộng/Sâu từ Group/Component.'); reset; return
          end
        ...
        def create_drawer(model,ox,oy,oz,w,d,h,t,bt,gl,gf,bo=0)
          ...
          model.abort_operation rescue nil; raise
        end
