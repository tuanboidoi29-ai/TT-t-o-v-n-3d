module TranTuan
  module TaoVan
    module Drawer
      ...
      def save_settings(vals); vals.each{|k,v|Sketchup.active_model.set_attribute(DICT,k,v.to_f)} end
      ...
      class TwoPointTool
        ...
        def onKeyDown(k,*_args)
          if k==TAB_KEY;Drawer.show_settings(self);return true end
          if k==27;Sketchup.active_model.select_tool(nil);return true end
          false
        end
        ...
        def local_points(a,b);return [a,b,nil] unless @container&&@container.valid?;tr=@container.transformation;[a.transform(tr.inverse),b.transform(tr.inverse),tr];end
        ...
        def add_part(g,name,x,y,z,w,d,h);return if w<=0||d<=0||h<=0;p=g.entities.add_group;p.name=name;f=p.entities.add_face([Geom::Point3d.new(x,y,z),Geom::Point3d.new(x+w,y,z),Geom::Point3d.new(x+w,y+d,z),Geom::Point3d.new(x,y+d,z)]);f.pushpull(h) if f;end
      end
    end
  end
end