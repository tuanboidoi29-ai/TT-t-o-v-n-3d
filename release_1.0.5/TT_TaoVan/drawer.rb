module TranTuan
  module TaoVan
    module Drawer
      ...
      class TwoPointTool
        def initialize
          @ip=Sketchup::InputPoint.new; @p1=nil; @p2=nil; @container=nil; @preview=nil
        ...
        def onMouseMove(_flags,x,y,view)
          ...
          if @p1
            ...
            Sketchup.set_status_text("TỰ ĐỘNG 2 ĐIỂM | ĐIỂM 1: MẶT TRÊN → ĐIỂM 2: ĐÁY | Cao: #{Drawer.mm_text(h)} | ĐƠN VỊ: mm | TAB = THỦ CÔNG",SB_VCB_LABEL)
          end
        end
        def draw(view)
          return unless @preview && !@preview.empty?; view.line_width=3; view.drawing_color=Sketchup::Color.new(255,128,0,255)
          if @manual_mode && @manual_ready && @manual_values
            ox=@manual_origin.x; oy=@manual_origin.y; oz=@manual_origin.z; w,d,h=@manual_values[0..2].map{|v| Drawer.mm(v)}; draw_box(view,box_points(ox,oy,oz,w,d,h)); return
          end
          if @preview.length==1
        ...
        def set_waiting_status(mode,detail)
          Sketchup.set_status_text("TT NGĂN KÉO | CHẾ ĐỘ CHỜ | #{mode} | ĐƠN VỊ: mm | TAB = ĐỔI CHẾ ĐỘ",SB_PROMPT); Sketchup.set_status_text(detail,SB_VCB_LABEL)
        end
        def update_status
          if @manual_mode
            set_waiting_status('THỦ CÔNG',@manual_ready ? 'ĐÃ NHẬP THÔNG SỐ (mm) → DI CHUỘT ĐỂ XEM PREVIEW → CLICK ĐỂ TẠO' : 'CLICK VỊ TRÍ ĐẶT → NHẬP THÔNG SỐ (mm) | TAB = TỰ ĐỘNG 2 ĐIỂM')
        ...
        def direct_container(ip); path=ip.instance_path; return nil unless path && path.respond_to?(:to_a); path.to_a.reverse_each{|e| return e if e.is_a?(Sketchup::Group)||e.is_a?(Sketchup::ComponentInstance)}; nil; end
        def same_container?(a,b); return false unless a && b && a.valid? && b.valid?; a.equal?(b) || (a.respond_to?(:persistent_id) && b.respond_to?(:persistent_id) && a.persistent_id==b.persistent_id); end
        ...
        def create_drawer(model,ox,oy,oz,w,d,h,t,bt,gl,gf,bo=0)
          # All dimension parameters entering this method are millimeters. Convert exactly once to SketchUp internal units.
