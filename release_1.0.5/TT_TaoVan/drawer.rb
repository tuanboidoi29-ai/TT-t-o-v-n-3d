    ...
    module Drawer
      ...
      def show_settings(tool = nil)
        ...
        @dialog ||= UI::HtmlDialog.new(
          ...
          preferences_key: 'TT_Drawer_Auto_123',
      ...
      def settings_html(d)
        fields = [
          ...
          ['gap_front', 'Hở trước'], ['depth_reserve', 'Chừa phía sau'], ['side_t', 'Độ dày hông'],
          ['back_t', 'Độ dày hậu'], ['bottom_t', 'Độ dày đáy'], ['front_t', 'Độ dày mặt trước']
      ...
      class TwoPointTool
        ...
        def activate
          status('AUTO: CLICK ĐIỂM 1 → RÊ CHÉO → CLICK ĐIỂM 2 TRÊN CÙNG MẶT | TAB: CÀI ĐẶT | ESC: THOÁT')
        end
        ...
        def apply_settings(cfg)
          @cfg = cfg
          reset
          status('ĐÃ LƯU → AUTO: CLICK ĐIỂM 1')
        end
        ...
        def onMouseMove(_flags, x, y, view)
          ...
          if @p1 && @frame
            d = measure(@p1, p)
            @preview_drawer = preview_box(@p1, p, d)
        ...
        def onLButtonDown(_flags, x, y, view)
          ...
          if @p1.nil?
            ...
            unless @face
              UI.messagebox('Điểm 1 phải nằm trên một Face.')
              reset
              return
            end
            @frame = build_frame(@p1, @face, @path)
            @container = best_container(@path)
        ...
        def draw(view)
          ...
          return unless box
          pts = box_points(box)
          view.drawing_color = Sketchup::Color.new(255, 128, 0, 180)
          view.line_width = 4
        ...
        def status(t)
          ...
          Sketchup.set_status_text(t, SB_VCB_LABEL)
        end

        def best_container(path)
          return nil unless path
          path.to_a.select { |e| e.is_a?(Sketchup::Group) || e.is_a?(Sketchup::ComponentInstance) }.max_by do |e|
            b = local_entity_bounds(e)
            b.width.to_f * b.depth.to_f * b.height.to_f
          end
        rescue
          nil
        end
        ...
        def build_frame(origin, face, path)
          ...
          normal = unit(face.normal.transform(tr))
          inward = normal.reverse

          # Z is the true vertical direction projected onto the selected face.
          z = Geom::Vector3d.new(0, 0, 1)
          z = z - normal * z.dot(normal)
          if z.length < 0.001
            # For a horizontal face, use a stable edge direction as the face-height axis.
            edge_vec = face.edges.map { |e| e.line[1] }.find { |v| v.length > 0.001 }
            z = edge_vec ? unit(edge_vec.transform(tr)) : Geom::Vector3d.new(1, 0, 0)
          else
            z = unit(z)
          end

          x = unit(z.cross(inward))
          z = unit(inward.cross(x))
        ...
        def frame_point(p)
          p.transform(@frame.inverse)
        end

        # The second click is locked to the plane of the first Face.
        # This prevents InputPoint snapping to another depth/edge from moving the preview.
        def locked_point(p)
          return p unless @p1 && @frame
          n = unit(@frame.yaxis)
          delta = p - @p1
          p - n * delta.dot(n)
        rescue
        ...
        # Probe from the CENTER of the selected rectangle, not from a corner.
        # This avoids hitting a side/corner object and gives a stable cabinet depth.
        def depth_from_region(a, b)
          xmin, xmax, zmin, zmax = normalized_region(a, b)
          local = Geom::Point3d.new((xmin + xmax) * 0.5, 0, (zmin + zmax) * 0.5)
          origin = local.transform(@frame)
          inward = unit(@frame.yaxis)
        ...
        def measure(a, b)
          xmin, xmax, zmin, zmax = normalized_region(a, b)
          w = (xmax - xmin).abs.to_mm
          h = (zmax - zmin).abs.to_mm
          d = depth_from_region(a, b)
        ...
        def preview_box(a, b, d)
          ...
          { origin: Geom::Point3d.new(x, Drawer.mm(@cfg['gap_front']), z), w: Drawer.mm(d[:dw]), d: Drawer.mm(d[:dd]), h: Drawer.mm(d[:dh]), tr: @frame }
        end
        ...
        def create(p2)
          ...
          begin
            xmin, _xmax, zmin, _zmax = normalized_region(@p1, p2)
            x = xmin + Drawer.mm(@cfg['rail_gap'])
            y = Drawer.mm(@cfg['gap_front'])
            z = zmin + Drawer.mm(@cfg['gap_bottom'])
            rw = Drawer.mm(d[:dw]); rd = Drawer.mm(d[:dd]); rh = Drawer.mm(d[:dh])
            st = Drawer.mm(@cfg['side_t']); bt = Drawer.mm(@cfg['bottom_t']); back = Drawer.mm(@cfg['back_t']); ft = Drawer.mm(@cfg['front_t'])

            g = model.entities.add_group
            g.name = 'TT - Ngăn kéo AUTO'
            add_part(g, 'Hông trái', x, y, z, st, rd, rh)
            add_part(g, 'Hông phải', x + rw - st, y, z, st, rd, rh)
            add_part(g, 'Đáy', x, y, z, rw, rd, bt)
            add_part(g, 'Mặt trước', x, y, z, rw, ft, rh)
            add_part(g, 'Hậu', x, y + rd - back, z, rw, back, rh)
            g.transform!(@frame)

            g.set_attribute(DICT, 'don_vi', 'mm')
            g.set_attribute(DICT, 'vung_rong_mm', d[:w])
            g.set_attribute(DICT, 'vung_cao_mm', d[:h])
            g.set_attribute(DICT, 'vung_sau_mm', d[:d])
            g.set_attribute(DICT, 'rong_mm', d[:dw])
            g.set_attribute(DICT, 'cao_mm', d[:dh])
            g.set_attribute(DICT, 'sau_mm', d[:dd])
            g.set_attribute(DICT, 'ray_moi_ben_mm', @cfg['rail_gap'])
            g.set_attribute(DICT, 'do_day_hong_mm', @cfg['side_t'])
            g.set_attribute(DICT, 'chua_sau_mm', @cfg['depth_reserve'])
