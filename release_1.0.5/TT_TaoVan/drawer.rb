module TranTuan
  module TaoVan
    module Drawer
      module_function
      TAB_KEY=9; DICT='TT_TaoVan_Drawer'
      ...
      class TwoPointTool
        ...
        def preview_box(a,b,d); return nil if d.values_at(:dw,:dh,:dd).any?{|v|v<=0}; xmin,xmax,zmin,zmax=normalized_region(a,b); {origin:Geom::Point3d.new(xmin+Drawer.mm(@cfg['rail_gap']),Drawer.mm(@cfg['gap_front']),zmin+Drawer.mm(@cfg['gap_bottom'])),w:Drawer.mm(d[:dw]),d:Drawer.mm(d[:dd]),h:Drawer.mm(d[:dh]),tr:@frame}; end
