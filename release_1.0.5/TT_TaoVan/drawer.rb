  ...
  module TaoVan
    module Drawer
      ...
      def show_settings(tool=nil)
        d=defaults
        @dialog ||= UI::HtmlDialog.new(dialog_title:'TT - CÀI ĐẶT NGĂN KÉO AUTO',preferences_key:'TT_Drawer_Auto_119',scrollable:true,resizable:true,width:450,height:650,style:UI::HtmlDialog::STYLE_DIALOG)
      ...
      class TwoPointTool
        ...
        def activate; status('AUTO: CLICK ĐIỂM 1 → CLICK ĐIỂM 2 TRÊN MẶT TRƯỚC | TAB: CÀI ĐẶT | ESC: THOÁT'); end
        ...
        def depth_from_ray(origin)
          ...
          # Fallback: local bounds of the selected cabinet/container.
          if @container && @container.valid?
            bb=local_entity_bounds(@container); tr=@path.transformation(@path.to_a.index(@container));
        ...
        def measure(a,b)
          q1=frame_point(a); q2=frame_point(b)
          w=(q2.x-q1.x).abs.to_mm; h=(q2.z-q1.z).abs.to_mm; d=depth_from_ray(a)
        ...
        def preview_box(a,b,d)
          ...
          q1=frame_point(a); x=q1.x+Drawer.mm(@cfg['rail_gap']); z=q1.z+Drawer.mm(@cfg['gap_bottom']); y=Drawer.mm(@cfg['gap_front'])
          {origin:Geom::Point3d.new(x,y,z),w:Drawer.mm(d[:dw]),d:Drawer.mm(d[:dd]),h:Drawer.mm(d[:dh]),tr:@frame}
        end

        def box_points(b)
          o=b[:origin]; x=b[:w]; y=b[:d]; z=b[:h]; a=[Geom::Point3d.new(o.x,o.y,o.z),Geom::Point3d.new(o.x+x,o.y,o.z),Geom::Point3d.new(o.x+x,o.y+y,o.z),Geom::Point3d.new(o.x,o.y+y,o.z),Geom::Point3d.new(o.x,o.y,o.z+z),Geom::Point3d.new(o.x+x,o.y,o.z+z),Geom::Point3d.new(o.x+x,o.y+y,o.z+z),Geom::Point3d.new(o.x,o.y+y,o.z+z)]; a.map{|p|p.transform(b[:tr])}
        end
        ...
        def create(p2)
          ...
          begin
            q=frame_point(@p1); x=q.x+Drawer.mm(@cfg['rail_gap']); y=Drawer.mm(@cfg['gap_front']); z=q.z+Drawer.mm(@cfg['gap_bottom']); rw=Drawer.mm(d[:dw]); rd=Drawer.mm(d[:dd]); rh=Drawer.mm(d[:dh]); st=Drawer.mm(@cfg['side_t']); bt=Drawer.mm(@cfg['bottom_t']); back=Drawer.mm(@cfg['back_t']); ft=Drawer.mm(@cfg['front_t'])
            g=model.entities.add_group; g.name='TT - Ngăn kéo AUTO'
            add_part(g,'Hông trái',x,y,z,st,rd,rh); add_part(g,'Hông phải',x+rw-st,y,z,st,rd,rh); add_part(g,'Đáy',x,y,z,rw,rd,bt); add_part(g,'Mặt trước',x,y,z,rw,ft,rh); add_part(g,'Hậu',x,y+rd-back,z,rw,back,rh); g.transform!(@frame)
