...
class UnsavedModel
  attr_accessor :path,:fail_copy,:fail_save
  attr_reader :copies,:saves
  def initialize(path='');@path=path;@copies=[];@saves=[];end
  def save(path)
    raise 'MUST NOT OVERWRITE EXISTING MODEL' unless self.path.empty?
    @saves<<path
    return false if fail_save
    File.binwrite(path,'current unsaved geometry and prepared scenes')
    self.path=path
    true
  end
  def save_copy(path)
    raise 'Model must be saved before copying.' if self.path.empty?
    @copies<<path
    File.binwrite(path,'current unsaved geometry and prepared scenes')
    !fail_copy
  end
...
Dir.mktmpdir('TT_test187_') do |dir|
  ENV['APPDATA']=File.join(dir,'Preferences')
  ...
  assert(File.file?(model.path) && model.saves==[model.path] && model.copies.empty?,'new model first saved once to persistent background file')
  initial=model.path
  assert(UI.panels.size==1 && UI.panels.first.last.end_with?('.pdf'),'only PDF output dialog shown')
  assert(File.file?(initial),'initial model kept after PDF export')
  ...
  # Brand-new model preview must also work; keep first-save files after every exit.
  T.fail_export=false
  fresh=UnsavedModel.new;Sketchup.active_model=fresh;T.instance_variable_set(:@model,fresh)
  T.run('preview',options);UI.drain
  assert(fresh.saves.length==1 && fresh.copies.empty?,'preview of untitled model bypasses unsupported save_copy')
  persistent=fresh.path
  assert(File.file?(persistent),'completed preview keeps the real model file')
  T.run('preview',options);T.finish_preview('cancelled');T.clear_preview_files;UI.drain
  assert(File.file?(persistent) && fresh.saves.length==1,'repeat preview and cancel never remove or overwrite initial save')
  fresh2=UnsavedModel.new;Sketchup.active_model=fresh2;T.instance_variable_set(:@model,fresh2)
  T.fail_export=true;T.run('preview',options);UI.drain
  assert(File.file?(fresh2.path),'render failure keeps newly saved model')
  assert(dialog.scripts.last.include?('Model mới đã được lưu nền tại:'),'model path change explicitly shown')
  T.fail_export=false
  failed=UnsavedModel.new;failed.fail_save=true;Sketchup.active_model=failed;T.instance_variable_set(:@model,failed)
  rejects('Không lưu được model nền'){T.run('preview',options)}
  assert(failed.copies.empty? && failed.path.empty?,'failed initial save never calls save_copy')
  T.clear_preview_files
end
