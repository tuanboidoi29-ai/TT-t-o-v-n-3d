...
const ids={};for(const k of ['views','project','scale','cut_scale','cut_mm','render','quality','stats','form','status','preview-list','preview-image','preview-label','preview-note','preview-screen','cancel-preview','dimensions','dim_offset','dim_font','stats_font','drawing','scope','title_font','detail_dims','preview-canvas'])ids[k]=new Element();
const fixed=new Element('select');fixed.disabled=true;
const fields=['project','scale','cut_scale','cut_mm','render','quality','stats','dimensions','dim_offset','dim_font','stats_font','drawing','scope','title_font','detail_dims'].map(k=>ids[k]);
...
const timers=[];const win={sketchup:bridge,addEventListener:()=>{}};const c=vm.createContext({document:doc,sketchup:bridge,window:win,setTimeout:f=>timers.push(f)});vm.runInContext(fs.readFileSync('output/layout189_ui.js','utf8'),c);
